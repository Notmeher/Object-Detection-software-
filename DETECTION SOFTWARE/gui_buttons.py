...
class Buttons:
    ...
    def add_button(self, text, x, y):
        ...
        # Adjust x-coordinate for the next button
        x += right_x + self.x_margin
    ...
    def display_buttons(self, frame):
        for b_index, button_value in self.buttons.items():
            ...
            if active:
                button_color = (0, 255, 0)  # Active button color (green)
                text_color = (255, 255, 255)  # Text color (white)
                thickness = -1
            else:
                button_color = (0, 0, 0)   # Inactive button color (Black)
                text_color = (255, 0, 0)
                thickness = 3
